...
void 	sprite_calc(t_all *all, int i)
{
	...
	all->sprt.start_y = all->res_y / 2 - all->sprt.height / 2;
	all->sprt.end_y = all->res_y / 2 + all->sprt.height / 2;
	if (all->sprt.start_y < 0)
		all->sprt.start_y = 0;
	if (all->sprt.end_y >= all->res_y)
		all->sprt.end_y = all->res_y - 1;
	//sprite width
	all->sprt.width = (int)fabs((all->res_y / all->sprt.modif_y));
	all->sprt.start_x =  all->sprt.screen_x - all->sprt.width / 2;
	all->sprt.end_x = all->sprt.screen_x + all->sprt.width / 2;
	if (all->sprt.start_x < 0)
		all->sprt.start_x = 0;
	if (all->sprt.end_x >= all->res_x)
		all->sprt.end_x = all->res_x - 1;
}
...
void 	sprite_draw(t_all *all)
{
	...
	while (all->sprt.start_x <= all->sprt.end_x)
	{
		tex_x = (int) (256 * (all->sprt.start_x - (-all->sprt.width / 2
				+ all->sprt.screen_x)) * all->tex[all->tex_id].width
				/ all->sprt.width) / 256;
		if (all->sprt.modif_y > 0
			&& all->sprt.start_x < all->res_x
			&& all->sprt.start_x > -1
			&& all->sprt.modif_y < all->sprt.depth_buf[all->sprt.start_x])
		{
			...
			{
				//printf("y %d\n", all->sprt.start_y);
				d = i * 256 - all->res_y * 128 + all->sprt.height * 128;
				...
				if ((color & 0x00FFFF) != 0)
					all->buf[i][all->sprt.start_x] = color;
				i++;
			}
		}
		all->sprt.start_x++;
